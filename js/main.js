...
document.addEventListener('DOMContentLoaded', function() {
  ...
  $.ajax({
      ...
      data: {
          key: 'JAUBZ-QUO65-GGNIX-IKRYS-UQGQJ-CABLN',
          output: 'jsonp',
      },
      dataType: 'jsonp',
...
function showWelcome() {
    ...
    let dist = getDistance(117.959681,27.656446, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  script.src = `https://apis.map.qq.com/ws/location/v1/ip?key=JAUBZ-QUO65-GGNIX-IKRYS-UQGQJ-CABLN&output=jsonp&callback=handleIpCallback`;
  document.body.appendChild(script);
});

// JSONP回调函数
window.handleIpCallback = function(res) {
...
class Cursor {
  ...
  create() {
    ...
    this.scr.innerHTML = `* {cursor: url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 8 8' width='8px' height='8px'><circle cx='4' cy='4' r='4' opacity='1.0' fill='rgb(46, 78, 126)'/></svg>") 4 4, auto}`;
  }
